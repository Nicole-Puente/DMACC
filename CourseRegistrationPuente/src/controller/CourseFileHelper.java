...
public class CourseFileHelper implements FileHelper {
	...
	public ArrayList<?> readFile() {
		...
		try {
			...
			while (fileIn.hasNextLine()) {
				...
				Instructor tempInstr = getInstructor(parts[5]);
				Classroom tempClass = getClassroom(parts[4]);

				Course currentCourse = new Course(parts[0], parts[1], parseTime(parts[2]), parseTime(parts[3]),
						tempInstr, tempClass);
			...
			System.out.println("No file called " + nameOfFile + " found when reading courses.");
	...
	public boolean writeFile(ArrayList<?> list) {
		...
		try {
			...
			for (Course c : coursesToWrite) {
				...
				sb.append(c.getCourseIdentifier() + "|" + c.getCourseName() + "|" + c.getStartTime() + "|"
						+ c.getEndTime() + "|" + c.getTeacher() + "|" + c.getLocation());
				courseFile.println(sb.toString());
			}

			courseFile.close();
		} catch (FileNotFoundException e) {
			System.out.println("No file called " + nameOfFile + " found when writing courses.");
			return false;
		}
		return true;
	}
	...
	private Classroom getClassroom(String string) {
		...
		Classroom returnedClassroom = new Classroom(parts[0], parts[1], parts[2], parts[3], Integer.parseInt(parts[4]));
	...
	private Instructor getInstructor(String string) {
		...
		Instructor returnedInstructor = new Instructor(parts[0], parts[1], parts[2]);
